...
struct BuildSentenceService: Sendable {
    ...
    func generate(_ request: Request) async throws -> [BuildSentenceQuestion] {
        ...
        return try usable(set.questions)
    }
    ...
    /// 풀 수 있는 문항만 남긴다.
    ///
    /// 예전에는 하나라도 규격을 어기면 세트 전체를 버렸는데, 실제로 돌려보니
    /// 10문항 중 1개꼴로 빈칸 수가 어긋난다. 그것 때문에 나머지 9개까지 날리면
    /// 사용자는 계속 "생성 실패"만 보게 된다.
    private func usable(_ questions: [BuildSentenceQuestion]) throws -> [BuildSentenceQuestion] {
        let kept = questions.filter(BuildSentenceEngine.isUsable)
        guard !kept.isEmpty else {
            throw APIError.decoding("풀 수 있는 Build a Sentence 문항을 받지 못했습니다.")
        }
        return kept
    }
