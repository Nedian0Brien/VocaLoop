...
AI 기본 provider는 서버의 Codex CLI이며 FastAPI `/api/ai/codex`가 `codex exec -m gpt-5.3-codex-spark`를 호출한다.
Gemini, OpenAI, Claude는 레거시/대체 provider로 남아 있으며, 이 경우에만 브라우저에서 사용자의 API 키로 provider API를 직접 호출한다.
현재 지원 provider와 모델 설정은 계정 설정 화면과 `/api/settings`에 저장된다.
...
`VITE_*_API_KEY`는 레거시/대체 direct API provider의 로컬 fallback 용도다.
기본 Codex CLI provider는 브라우저 API 키를 요구하지 않는다.
...
- 기본 provider는 Codex CLI이며 모델은 `gpt-5.3-codex-spark`다.
- Codex CLI는 서버의 Codex 인증/설정을 사용하므로 계정별 API 키 입력이 필요 없다.
- Gemini/OpenAI/Claude를 대체 provider로 선택할 때만 계정별 API 키를 프로필 탭에서 설정한다.
- 서버 공용 DEFAULT_GEMINI_API_KEY fallback은 사용하지 않는다.
- direct API provider에서 키가 없으면 단어 분석, AI 채점, TOEFL 문제 생성 기능은 사용할 수 없다.
