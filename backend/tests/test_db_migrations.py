...
def test_bootstrap_db_adds_missing_word_flag_column(monkeypatch, tmp_path):
    db_path = tmp_path / "legacy-word-flags.db"
    conn = sqlite3.connect(db_path)
    conn.executescript(
        """
        CREATE TABLE users (
        ...
        INSERT INTO users (id, email, display_name, password_hash)
        VALUES (1, 'legacy-flag@example.com', 'Legacy Flag User', 'hash');
        INSERT INTO words (user_id, word, meaning_ko)
        VALUES (1, 'candid', '솔직한');
        """
    )
    ...
    conn = sqlite3.connect(db_path)
    columns = {row[1] for row in conn.execute("PRAGMA table_info(words)").fetchall()}
    is_flagged = conn.execute("SELECT is_flagged FROM words WHERE word = 'candid'").fetchone()[0]
    conn.close()
